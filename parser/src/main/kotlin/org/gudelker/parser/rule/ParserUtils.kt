    ...
    fun parseOptionalType(stream: TokenStream): TypeParseResult {
        if (stream.check(TokenType.COLON)) {
            ...
            return if (typeToken == null) {
                TypeParseResult(null, ParserSyntaxError("Se esperaba un tipo después de ':'"), streamAfterColon)
            } else {
                TypeParseResult(typeToken.getValue(), null, streamAfterType)
            }
        }
        return TypeParseResult(null, null, stream)
    }
