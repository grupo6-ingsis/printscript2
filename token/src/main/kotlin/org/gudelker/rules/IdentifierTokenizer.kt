...
class IdentifierTokenizer : RuleTokenizer {
    override fun matches(actualWord: String, nextChar: Char?): Boolean {
        ...
        return matchesRegex(actualWord, regex) && !matchesRegex(nextChar.toString(), regex)
    }

    private fun matchesRegex(actualWord: String, regex: String) = actualWord.toRegex().matches(regex)
